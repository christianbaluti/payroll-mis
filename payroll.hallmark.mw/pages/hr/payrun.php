...
<html lang="en">
  ...
  <script type="text/javascript">
      ...
      function applyBulkHours() {
          ...
          Swal.fire('Success!', 'Bulk overtime hours applied.', 'success');
      }
      ...
      async function generatePayroll() {
          const selectedEmployees = document.querySelectorAll('.employeeCheckbox:checked');
          if (selectedEmployees.length === 0) {
              Swal.fire('Error!', 'Please select at least one employee to generate payroll.', 'error');
          ...
          const payrollData = {
              // Assuming payrollMonth and year are selected elsewhere or determined
              // For now, using current month/year or a placeholder
              month: new Date().getMonth() + 1, // Current month (1-indexed)
              year: new Date().getFullYear(), // Current year
              unique_id: Date.now().toString(), // Simple unique ID
              status: 'drafted',
              employees: []
          };
          ...
          selectedEmployees.forEach(checkbox => {
              ...
              payrollData.employees.push({
                  ...
                  weekend_overtime_pay: parseFloat(row.querySelector('.weekend-ot-pay').textContent.replace('MWK ', '')) || 0,
                  weekday_overtime_pay: parseFloat(row.querySelector('.weekday-ot-pay').textContent.replace('MWK ', '')) || 0,
          ...
          // Include total values
          payrollData.totals = {
              totalGross: parseFloat(document.getElementById("totalGrossDisplay").textContent.replace('MWK ', '') || 0),
              totalNet: parseFloat(document.getElementById("totalNetDisplay").textContent.replace('MWK ', '') || 0),
              totalPaye: parseFloat(document.getElementById("totalPayeDisplay").textContent.replace('MWK ', '') || 0),
              totalAdditions: parseFloat(document.getElementById("totalAdditionsDisplay").textContent.replace('MWK ', '') || 0),
              totalDeductions: parseFloat(document.getElementById("totalDeductionsDisplay").textContent.replace('MWK ', '') || 0),
              totalPension: parseFloat(document.getElementById("totalPensionDisplay").textContent.replace('MWK ', '') || 0),
              totalWelfareFund: parseFloat(document.getElementById("totalWelfareFund").textContent.replace('MWK ', '') || 0),
              totalWeekendOvertimePay: parseFloat(document.getElementById("totalWeekendOvertime").textContent.replace('MWK ', '') || 0),
              totalWeekdayOvertimePay: parseFloat(document.getElementById("totalWeekdayOvertime").textContent.replace('MWK ', '') || 0),
              totalOvertimePay: parseFloat(document.getElementById("totalOvertime").textContent.replace('MWK ', '') || 0)
          };

          try {
              const response = await fetch('generate_payroll3.php', {
                  method: 'POST',
                  headers: {
              ...
              if (result.success) {
                  ...
                  Swal.fire('Error!', result.message || 'Failed to generate payroll', 'error');
              }
          } catch (error) {
              console.error('Error generating payroll:', error);
              hideLoading(); // Hide loading overlay
              Swal.fire('Error!', 'Failed to generate payroll', 'error');
          }
      }
      ...
      function updateTotals() {
        ...
        rows.forEach(row => {
          ...
          if (checkbox && checkbox.checked) {
            ...
            // FIX: Correctly parse the currency string before converting to float
            const weekendOTPay = parseFloat(row.querySelector('.weekend-ot-pay').textContent.replace('MWK ', '') || 0);
            const weekdayOTPay = parseFloat(row.querySelector('.weekday-ot-pay').textContent.replace('MWK ', '') || 0);
      ...
      window.applyBulkChanges = function () {
        ...
        selectedCheckboxes.forEach(cb => {
          ...
          let taxableIncome = salary + additions - deductions;
          let paye = calculatePAYE(taxableIncome);
          let netSalaryBeforePension = salary - paye;
          let pension = netSalaryBeforePension * 0.05; // Recalculate pension based on new net before pension
          let welfareFund = parseFloat(row.dataset.welfareFund || 0); // Assuming fixed welfare
          let totalOTPay = parseFloat(row.dataset.totalOvertime || 0); // Use calculated overtime pay

          let baseNet = netSalaryBeforePension - pension - welfareFund;
          let currentNet = baseNet + totalOTPay + additions - deductions;
        ...
        updateTotals();
        closeBulkModal();
        Swal.fire('Success!', "Bulk updates applied successfully!", 'success');
      }
